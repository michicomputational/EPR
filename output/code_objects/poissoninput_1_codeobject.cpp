...
namespace {
    ...
    long poissoninput_binomial_59(const int vectorisation_idx)
    {
        long X = 0;
        double px = 0.134793581210640;
        double U = _rand(vectorisation_idx);
        while (U > px)
        {
            X++;
            if (X > 19.297398648351724)
            {
                X = 0;
                px = 0.134793581210640;
                U = _rand(vectorisation_idx);
            } else
            {
                U -= px;
                px = ((500-X+1) * 0.004000000000000 * px)/(X*0.996000000000000);
            }
        }
        return X;
    }
...
const size_t _numnot_refractory = 10000;
    ...
    for(int _idx=0; _idx<_N; _idx++)
    {
        ...
        if(not_refractory)
            v += _lio_1 * poissoninput_binomial_59(_vectorisation_idx);
